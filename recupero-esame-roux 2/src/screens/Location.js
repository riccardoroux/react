...
import { getLocation } from "../api/api"; // Modifica l'import di getLocation
...
const List = ({ navigation }) => {
  ...
  useEffect(() => {
    getLocation().then((response) => setList(response));
  }, []);
    ...
    return (
      <View style={[styles.card, { width: screenWidth - 40 }]}>
        ...
        <View style={styles.detailsContainer}>
          ...
          <Text style={styles.details}>{`Dimension: ${item.dimension}`}</Text>
        </View>
      </View>
    );
  };
